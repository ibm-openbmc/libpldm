...
{
	uint32_t l_record_handle;

	if (!repo || !data || !size) {
		return -EINVAL;
	}

	l_record_handle = record_handle ? *record_handle : 0;

	pldm_pdr_record *record = make_new_record(
		repo, data, size, l_record_handle, is_remote, terminus_handle);
				      ...
				      uint32_t *bmc_record_handle, bool hotplug)
{
	...
	if (hotplug) {
		*bmc_record_handle = pldm_pdr_add_hotplug_record(
...
{
	return pldm_entity_node_get_remote_container_id(entity);
}
...
uint32_t pldm_entity_association_pdr_remove_contained_entity(
	pldm_pdr *repo, pldm_entity entity, uint8_t *event_data_op,
	bool is_remote)
{
	assert(repo != NULL);
	...
	*event_data_op = PLDM_RECORDS_MODIFIED;
	...
	while (record != NULL) {
		...
		/*hdr->type == PLDM_PDR_ENTITY_ASSOCIATION)*/ {
			...
			if (!new_pdr->num_children) // record will be deleted and
					...
					uint8_t association_type)
{
	...
	node->entity = entity;
	node->parent = parent;
	node->remote_container_id = remote_container_id;
 ...
 * entity
 */
static int pldm_entity_association_find_record_hndl_by_entity(
	pldm_pdr *repo, pldm_entity *entity, bool is_remote,
	uint32_t *record_handle)
{
	if (!repo || !entity || !record_handle) {
		return -EINVAL;
	}
	uint16_t *entity_type = NULL;
	uint16_t *entity_instance_num = NULL;
	uint16_t *entity_container_id = NULL;
	uint8_t *num_children = NULL;
	uint8_t *hdr_type = NULL;
	int rc = 0;
	pldm_pdr_record *record = repo->first;
	...
	while (record != NULL) {
		...
		pldm_msgbuf_span_required(dst, sizeof(uint8_t),
					  (void **)(&hdr_type));
		if (record->is_remote == is_remote &&
		    *hdr_type == PLDM_PDR_ENTITY_ASSOCIATION) {
			...
			for (int i = 0; i < *num_children; i++) {
				...
				pldm_msgbuf_span_required(
					dst, sizeof(uint16_t),
					(void **)(&entity_instance_num));
				pldm_msgbuf_span_required(
					dst, sizeof(uint16_t),
					(void **)(&entity_container_id));
...
LIBPLDM_ABI_TESTING
int pldm_entity_association_remove_contained_entity(pldm_pdr *repo,
						    pldm_entity *entity,
						    bool is_remote,
						    uint32_t *pdr_record_handle)
{
	...
	rc = pldm_entity_association_find_record_hndl_by_entity(
	...
	//Add all children of original PDR to new PDR
	for (int i = 0; i < num_children - 1; i++) {
		pldm_msgbuf_span_required(src, sizeof(uint16_t),
					  (void **)(&entity_type));
		pldm_msgbuf_span_required(src, sizeof(uint16_t),
					  (void **)(&entity_instance_num));
		pldm_msgbuf_span_required(src, sizeof(uint16_t),
